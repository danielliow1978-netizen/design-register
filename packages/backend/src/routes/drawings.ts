...
router.post('/:id/approve', requireAuth, requireMinRole('DESIGN_MANAGER'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Fetch active Department Head, Design Manager and Admin users to CC on all approval emails
    const deptHeads = await prisma.user.findMany({
      where: { role: { in: ['DEPARTMENT_HEAD', 'DESIGN_MANAGER', 'ADMIN'] }, active: true },
