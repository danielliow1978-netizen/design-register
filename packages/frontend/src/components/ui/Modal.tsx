...
export function Modal({ open, onClose, children, maxWidth = 'max-w-lg' }: ModalProps) {
  ...
  return (
    ...
    >
      <div className={`bg-surface rounded-lg border border-border w-full ${maxWidth} shadow-xl max-h-[90vh] flex flex-col`}>
        {children}
      </div>
    </div>
  )
...
export function ModalFooter({ children }: { children: ReactNode }) {
  return <div className="flex justify-end gap-2 p-6 pt-4 border-t border-border shrink-0">{children}</div>
}
